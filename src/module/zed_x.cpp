...
const viam::sdk::Model ZedX::model{"viam", "zed", "zedx"};

std::vector<std::string> ZedX::validate(viam::sdk::ResourceConfig /*cfg*/) { return {}; }

ZedX::Config ZedX::parse_config(const viam::sdk::ResourceConfig& cfg) {
  Config out;
  ...
  if (res_it != attrs.end()) {
    if (const auto* s = res_it->second.get<std::string>()) {
      if (*s == "hd1200") {
        out.resolution = sl::RESOLUTION::HD1200;
      } else if (*s == "hd1080") {
        out.resolution = sl::RESOLUTION::HD1080;
      } else if (*s == "svga") {
        out.resolution = sl::RESOLUTION::SVGA;
      } else {
        throw std::invalid_argument("resolution must be one of: hd1200, hd1080, svga (got: " + *s +
                                    ")");
      }
    }
  }
...
ZedX::ZedX(viam::sdk::Dependencies /*deps*/, viam::sdk::ResourceConfig cfg)
...
viam::sdk::Camera::image_collection ZedX::get_images(std::vector<std::string> filter_source_names,
...
viam::sdk::Camera::raw_image ZedX::convert_zed_depth_to_viam(const sl::Mat& depth_mat) {
...
viam::sdk::Camera::point_cloud ZedX::get_point_cloud(std::string /*mime_type*/,
                                                     const viam::sdk::ProtoStruct& /*extra*/) {
  std::lock_guard<std::mutex> lock(grab_mu_);
...
viam::sdk::Camera::point_cloud ZedX::encode_zed_cloud_to_pcd(const sl::Mat& cloud) {
...
viam::sdk::Camera::properties ZedX::get_properties() {
  std::lock_guard<std::mutex> lock(grab_mu_);
...
viam::sdk::ProtoStruct ZedX::get_status() {
  throw std::runtime_error("get_status not implemented");
}

std::vector<viam::sdk::GeometryConfig> ZedX::get_geometries(
    const viam::sdk::ProtoStruct& /*extra*/) {
  return {};
}

viam::sdk::ProtoStruct ZedX::do_command(const viam::sdk::ProtoStruct& /*command*/) {
